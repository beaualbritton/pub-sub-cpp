...
{
  public:
  WebSocketServer(asio::io_context& ioc, tcp::endpoint endpoint) : Server(ioc, endpoint), broker_client(ioc) { set_forward_handler(); broker_client.run("broker", "6767");}; // run broker 
